...
let datosContenedor = new Contenedor("productos.txt");
...
app.get('/productos', async (req, res) => {
    ...
    res.json(await datosContenedor.getAll())
})

app.get('/productoRandom', async (req, res) => {

    const arrProductos = await datosContenedor.getAll();
    let randBetween = Math.floor(Math.random() * arrProductos.length) + 1;

    res.json(await datosContenedor.getById(randBetween))
})